...
| O02 | Windows Defender real-time-scan exclusion for `D:/Douglas/OUTROS/CRAEI_data` and `D:/Douglas/OUTROS/CRAEI_isimip_raw_cache` (item 6) — needed to fully rule out Defender as a contributor to the `crop_to_countries` "Permission denied" retries (see D26) and to the paused-run hang. Not applied: this session has no admin privilege (`Get-MpPreference` refused even read access). Command for the author to run in an elevated PowerShell: `Add-MpPreference -ExclusionPath "D:\Douglas\OUTROS\CRAEI_data","D:\Douglas\OUTROS\CRAEI_isimip_raw_cache","C:\Users\User\AppData\Local\CRAEI_staging"`. | open | author, COMANDO 12 rework, item 6 | 2026-09-18 |
